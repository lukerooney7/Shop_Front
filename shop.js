...
function generateStockItem( id, photo, name, comment, price, quantity, total, report ) {
		...
		if( report ) {
			...
			strVar += "    <item_quantity><input name=\"" + id + "\" type=\"text\" value=\"0\" pattern=\"[0-9]+\" size=\"3\" onchange=\"updateCosts(this,'" + id + "');\" \/><\/item_quantity>";
		}
...
function generateOrderSubmitInput() {
	...
	strVar += "<input type=\"submit\" name=\"submit\" value=\"Press here to place your order.\" \/ onclick=\"submitPressed()\">";
...
function generateOrderBody() {
	...
	strVar += "<hr \/>";
	strVar += generateForm();
	strVar += "<script src=\"CS2003-P2\/shopfront.js\" type=\"text/javascript\"><\/script>";
	strVar += "<\/body>";
